...
module.exports = function(AV) {
  ...
  AV._getInstallationId = function() {
    ...
    if (AV.Storage.async) {
      AV.Storage.getItem(path)
        .then(function(installationId) {
            AV._installationId = installationId;
            AV._genInstallationId(path);
          });
    } else {
      AV._installationId = AV.Storage.getItem(path);
      AV._genInstallationId(path);
    }
  ...
  AV._genInstallationId = function(path) {
    if (!AV._installationId || AV._installationId === "") {
      ...
      AV.Storage.setItem(path, AV._installationId);
    }
  };
